...
namespace Veganimus.Platformer
{
	public class CameraController : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			float windowAnchorX = _cameraPosition.x - movementWindowSize.x / 2 + windowOffset.x;
			float windowAnchorY = _cameraPosition.y - movementWindowSize.y / 2 + windowOffset.y;

			//From our anchor point, we set the size of the window based on the public variable above.
			_windowRect = new Rect(windowAnchorX, windowAnchorY, movementWindowSize.x, movementWindowSize.y);
		...
		private void CameraUpdate()
		{
			...
			if (activeTracking && _playerPosition != _previousPlayerPosition)
			{
				...
				_windowRect.x = _cameraPosition.x - movementWindowSize.x / 2 + windowOffset.x;
				_windowRect.y = _cameraPosition.y - movementWindowSize.y / 2 + windowOffset.y;
				...
				if (!_windowRect.Contains(_playerPosition))
				{
					...
					_cameraPosition.x += DifferenceOutOfBounds(positionDifference.x, movementWindowSize.x);


					_cameraPosition.y += DifferenceOutOfBounds(positionDifference.y, movementWindowSize.y);
				...
				{
					_cameraPosition.y = Mathf.Clamp(_cameraPosition.y, limitBottom, limitTop);
					_cameraPosition.x = Mathf.Clamp(_cameraPosition.x, limitLeft, limitRight);
				}
		...
		private void DrawDebugBox()
		{
			...
			_windowRect.x = cameraPos.x - movementWindowSize.x / 2 + windowOffset.x;
			_windowRect.y = cameraPos.y - movementWindowSize.y / 2 + windowOffset.y;
		...
		//-----------------------------------------------------------------------------
		//  CameraController cameraScript = Camera.main.GetComponent<CameraControl>();
